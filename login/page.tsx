      ...
      if (signInError) {
        ...
        if (
          ...
          const { data: clientCheck } = await supabase
            ...
            .select('id, name')
            .or(
              `business_number.eq.${bizNoClean},business_no.eq.${bizNoClean},business_number.eq.${bizOriginal},business_no.eq.${bizOriginal}`,
            )
            .maybeSingle();
          ...
          if (clientCheck || isAdminBiz) {
            ...
            if (signUpData?.user) {
              ...
              await supabase
                ...
                .or(
                  `business_number.eq.${bizNoClean},business_no.eq.${bizNoClean}`,
                );
            }
  ...
  const linkOrCreateClient = async (params: {
    ...
    try {
      ...
      const { data: existing } = await supabase
        ...
        .select('*')
        .or(
          `business_number.eq.${bizClean},business_no.eq.${bizClean},business_number.eq.${bizOriginal},business_no.eq.${bizOriginal}`,
        )
        .maybeSingle();
